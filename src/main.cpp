...
int cmd_list(const std::vector<std::string>& args) {
    ...
    if (sort_key == "rss") {
        ...
        std::sort(processes.begin(), processes.end(),
                  [](const auto& a, const auto& b) { return a.cpu_seconds() > b.cpu_seconds(); });
...
int cmd_show(int pid) {
    ...
    print_field(std::cout, "PID / PPID", std::to_string(p.pid) + " / " + std::to_string(p.ppid));
    ...
    if (p.voluntary_ctxt >= 0) {
        // Voluntary: the task blocked itself (I/O, lock). Involuntary: preempted by the scheduler.
        print_field(std::cout, "Ctx switches (voluntary)", std::to_string(p.voluntary_ctxt));
        print_field(std::cout, "Ctx switches (involuntary)", std::to_string(p.nonvoluntary_ctxt));
    }
    ...
    if (!regions.empty()) {
        ...
        print_field(std::cout, "Anonymous + special", human_bytes(static_cast<long long>(anon)));
...
int cmd_threads(int pid) {
    ...
    for (const auto& t : threads) {
        table.add_row({std::to_string(t.tid), std::string(1, t.state),
                       human_duration(t.cpu_seconds()), std::to_string(t.last_cpu),
                       std::to_string(t.priority), std::to_string(t.nice),
                       std::to_string(t.voluntary_ctxt), std::to_string(t.nonvoluntary_ctxt),
                       t.comm});
    }
    table.print(std::cout);
    std::cout << "\n" << threads.size() << " threads, each under /proc/" << pid
              << "/task/<tid>.\n"
...
int cmd_maps(int pid) {
    ...
    for (const auto& r : regions) {
        ...
        table.add_row({hex_address(r.start), hex_address(r.end), r.perms,
                       human_bytes(static_cast<long long>(r.size())), hex_address(r.offset), path});
    }
    table.print(std::cout);
    ...
    std::vector<std::pair<std::string, unsigned long long>> sorted(by_path.begin(), by_path.end());
...
int cmd_mem() {
    ...
    print_field(std::cout, "Active / Inactive", human_kb(m.active) + " / " + human_kb(m.inactive));
...
int main(int argc, char** argv) {
    ...
    if (command == "show" || command == "threads" || command == "maps") {
        ...
        if (command == "show") return cmd_show(pid);
        if (command == "threads") return cmd_threads(pid);
        return cmd_maps(pid);
    }
