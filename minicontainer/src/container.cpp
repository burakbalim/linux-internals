...
bool setup_rootfs(const std::string& rootfs, std::string* error) {
    ...
    if (::mount(nullptr, "/", nullptr, MS_REC | MS_PRIVATE, nullptr) != 0) {
        if (error) *error = errno_message("mount(/, MS_REC|MS_PRIVATE)");
        return false;
    }
    ...
    if (::mount(rootfs.c_str(), rootfs.c_str(), nullptr, MS_BIND | MS_REC, nullptr) != 0) {
        if (error) *error = errno_message("bind mount " + rootfs);
        return false;
    }

    const std::string put_old = rootfs + "/old_root";
    if (::mkdir(put_old.c_str(), 0700) != 0 && errno != EEXIST) {
        if (error) *error = errno_message("mkdir " + put_old);
        return false;
    }

    if (pivot_root(rootfs.c_str(), put_old.c_str()) != 0) {
        if (error) *error = errno_message("pivot_root");
        return false;
    }

    if (::chdir("/") != 0) {
        if (error) *error = errno_message("chdir(/)");
        return false;
    }
    ...
    if (::umount2("/old_root", MNT_DETACH) != 0) {
        if (error) *error = errno_message("umount2(/old_root)");
        return false;
    }
    ::rmdir("/old_root");
    return true;
...
bool mount_proc(std::string* error) {
    if (::mkdir("/proc", 0555) != 0 && errno != EEXIST) {
        if (error) *error = errno_message("mkdir /proc");
        return false;
    }
    ...
    if (::mount("proc", "/proc", "proc", 0, nullptr) != 0) {
        if (error) *error = errno_message("mount /proc");
        return false;
    }
    return true;
}
...
int run_container(const ContainerConfig& cfg, std::string* error) {
    ...
    if (pid < 0) {
        if (error) *error = errno_message("clone");
        return -1;
    }
    ::close(ready_pipe[0]);
    ...
    if (cfg.memory_limit > 0 || cfg.pids_limit > 0) {
        ...
        if (!cgroup.create("mc-" + std::to_string(pid), &cgroup_error)) {
            ...
            if (cfg.memory_limit > 0 && !cgroup.set_memory_limit(cfg.memory_limit, &cgroup_error)) {
    ...
    while (::waitpid(pid, &status, 0) < 0) {
        if (errno != EINTR) {
            if (error) *error = errno_message("waitpid");
            return -1;
        }
    }
