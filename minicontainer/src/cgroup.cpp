...
bool vacate_root(std::string* error) {
    ...
    if (::mkdir(leaf.c_str(), 0755) != 0 && errno != EEXIST) {
        if (error) *error = errno_message("mkdir " + leaf);
        return false;
    }
...
bool enable_controllers(const std::string& dir, std::string* error) {
    ...
    if (!available) {
        if (error) *error = "cannot read " + dir + "/cgroup.controllers";
        return false;
    }
    ...
    if (enable.empty()) {
        if (error) *error = "neither memory nor pids controller is available in " + dir;
        return false;
    }
...
bool Cgroup::create(const std::string& name, std::string* error) {
    ...
    if (::mkdir(kParent, 0755) != 0 && errno != EEXIST) {
        if (error) *error = errno_message(std::string("mkdir ") + kParent);
        return false;
    }
    ...
    if (::mkdir(dir.c_str(), 0755) != 0 && errno != EEXIST) {
        if (error) *error = errno_message("mkdir " + dir);
        return false;
    }
